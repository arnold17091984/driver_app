...
func (r *UserRepo) GetByEmployeeID(ctx context.Context, employeeID string) (*model.User, error) {
	...
	err := r.db.GetContext(ctx, &user,
		`SELECT id, employee_id, password_hash, name, role, priority_level, fcm_token, is_active, created_at, updated_at
...
func (r *UserRepo) GetDriversByVehicleIDs(ctx context.Context, vehicleIDs []string) ([]model.User, error) {
	query, args, err := sqlx.In(
		`SELECT u.id, u.employee_id, u.password_hash, u.name, u.role, u.priority_level, u.fcm_token, u.is_active, u.created_at, u.updated_at
		 FROM users u
